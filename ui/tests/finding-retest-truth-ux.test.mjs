...
test('finding surfaces distinguish canonical proof from latest retest', () => {
  ...
  assert.match(detail, /Canonical finding proof:/)
  assert.match(detail, /Finding proof/)
  assert.match(detail, /Latest replay/)
  assert.match(detail, /primary tested endpoint:/)
  assert.match(detail, /Tested scope \(/)
  assert.match(detail, /Latest advisory AI retest/)
  assert.match(detail, /cannot override the canonical deterministic proof state/)
})
