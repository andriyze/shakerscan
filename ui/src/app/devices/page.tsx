...
export default function DevicesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [deviceData, policyData, readiness] = await Promise.all([
        getDevices({ search: search || undefined }), getDevicePolicies(), getDeviceReadiness(),
      ])
  ...
  async function startScan() {
    ...
    try {
      ...
      setScanTarget(null)
      toast.success('Device scan queued', { link: { href: `/devices/${scanTarget.id}?scan=${queued.scan_id}`, label: 'View device' } })
      await load()
    } catch (error) {
