...
test('canonical proof requires the authoritative boolean and matching state', () => {
  ...
  assert.equal(canonicalFindingProofVerified({ is_verified: false, proof_state: 'verified' }), true)
  assert.equal(canonicalFindingProofVerified({ proof_state: 'verified' }), true)
})
