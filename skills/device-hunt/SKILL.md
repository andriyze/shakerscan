...
7. If a bound web credential or imported request contains secrets and the device uses untrusted HTTPS, keep the default credential-withholding behavior. Only pass `allow_untrusted_tls_credentials=true` when the user explicitly accepts that network-interception risk at session creation; the planner cannot enable it later.
...
5. Queue the smallest useful deterministic scan: inventory before posture, posture before thorough. Treat ports as the route to application evidence, not the final objective: inspect the platform/API surface, authentication boundaries, deterministic findings, and imported-request comparisons. Set `include_imported_requests=true` when the redacted inventory is relevant. Imported sockets remain pinned to discovered device origins; Postman scripts, HAR responses, and external OpenAPI references never execute. Request `ssh-authenticated-host-review` only through the declared `capability_ids` field under an authenticated session with a bound SSH profile.
...
- Never use local-host shell, raw sockets, arbitrary URLs, agent-supplied credentials, cross-device pivots, multicast discovery, or credential guessing through ordinary tools. Pairing, control, update, and reset requests are testable only when they are exact user-bound imported requests under fixed authenticated-active and state-changing authority, or part of a separately confirmed immutable SSH plan. Disclose their effects accurately and never claim confirmation before the UI records it.
