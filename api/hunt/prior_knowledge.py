...
_GUIDANCE = (
    ...
    "START WITH kind='endpoint_groups': the same inventory collapsed into route templates, so one "
    "handler answering hundreds of junk parameters is one row instead of hundreds. Each group "
    "carries route_template, grouping_evidence, sample_count, representatives, principal_contexts, "
    "prior_results, open_questions and sample_ids -- pass a sample_id back to kind='endpoints' "
    "with filter.id to drill into any member, so no lead is lost by grouping. Treat "
    "grouping_evidence='homogeneous_siblings' as inferred from agreeing responses, not declared. "
