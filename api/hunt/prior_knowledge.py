...
_GUIDANCE = (
    ...
    "POST /hunts/{hunt_id}/query before spending crawl or discovery budget. "
    "For an opt-in grouped overview use kind='endpoint_groups'. Equal status codes never merge "
    "literal routes; grouping_inferred means identifier-shaped grouping, not a proven handler. "
    "Method, auth_state, param_shape, param_location and content_type remain distinct. "
    "sample_count counts distinct sampled paths per variant; member_count and sample_ids retain all input rows "
    "and prior_results retain duplicate evidence. Representative IDs drill into kind='endpoints' "
    "with filter.id. Active leads and unexplored groups alternate ahead of settled work. "
    "Filters apply before grouping. inventory_truncated means the overview is incomplete: narrow "
    "filters or page the raw endpoints. If a grouped cursor reports changed inventory, restart "
    "without the cursor; do not treat the error as an empty page. "
    "kind='endpoints' still returns the individual samples ranked by priority_score with "
    "param_shape, auth_state and "
    ...
    "findings whose last_verification_verdict is 'exploited' have prior proof. Avoid redundant "
    "reproduction unless the operator requests a retest, the target changed, a different principal "
    "matters, or an attack-chain hypothesis needs it. Query kind='service_intelligence' before "
    "repeating port/service discovery; "
