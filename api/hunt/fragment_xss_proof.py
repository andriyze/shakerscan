"""Route a single-parameter Hunt XSS proof to the pinned browser when needed.
...
``xss.verify``. An explicit deep browser request can also use it for one unambiguous
server-query parameter; Dalfox's broad payload scan remains the non-browser path.
"""
...
def hunt_browser_xss_proof_adapter(
    *,
    ...
    """Return a pinned-browser XSS prover for one fragment or explicit deep query.
