...
def valid_marker(value:Any)->bool: return isinstance(value,str) and bool(MARKER_RE.fullmatch(value)) and len(set(value[4:]))>=8
def canonical_hash(value:Any)->str:
 raw=json.dumps(value,sort_keys=True,separators=(",",":"),ensure_ascii=False); return "sha256:"+hashlib.sha256(raw.encode()).hexdigest()
...
def relative_path(value:Any)->str:
 if not isinstance(value,str) or not value.startswith("/") or len(value)>1024: raise ContractError("endpoint_must_be_a_bounded_relative_path")
 p=urlsplit(value)
 if p.scheme or p.netloc or p.query or p.fragment or any(ch in value for ch in ("%","\\","\r","\n","?","#")) or "//" in value or any(x in {".",".."} for x in value.split("/")): raise ContractError("unsafe_endpoint_path")
 if not re.fullmatch(r"/[A-Za-z0-9_./{}-]*",value): raise ContractError("unsupported_endpoint_path")
 return value
def _name(value:Any)->str:
 if not isinstance(value,str) or not NAME_RE.fullmatch(value): raise ContractError("invalid_identifier")
 return value
def _field(value:Any)->str:
 if not isinstance(value,str) or len(value)>128 or not PATH_RE.fullmatch(value): raise ContractError("invalid_response_path")
 return value
def _keys(value:Any,allowed:set[str],required:set[str])->dict[str,Any]:
 if not isinstance(value,dict) or set(value)-allowed or required-set(value): raise ContractError("missing_or_unknown_contract_fields")
 return value
@dataclass(frozen=True)
class Identity: role:str; subject:str; tenant:str; resource_id:str
@dataclass(frozen=True)
class BoundaryContract:
 name:str; owner:Identity; attacker:Identity; identity_path:str; subject_field:str; tenant_field:str; resource_path:str; resource_id_field:str; resource_owner_field:str; resource_tenant_field:str; marker_field:str; response_path:str; baseline_prompt:str; attacks:tuple[str,...]; repetitions:int; action:Any; source:dict[str,Any]
 @property
 def digest(self)->str: return canonical_hash(self.source)
 @classmethod
 def parse(cls,raw:Any)->"BoundaryContract":
  raw=_keys(raw,{"version","name","owner","attacker","identity","resource","response_path","baseline_prompt","attacks","repetitions","action"},{"version","name","owner","attacker","identity","resource","response_path"})
  ...
  ident=_keys(raw["identity"],{"path","subject_field","tenant_field"},{"path","subject_field","tenant_field"}); resource=_keys(raw["resource"],{"path","id_field","owner_field","tenant_field","marker_field"},{"path","id_field","owner_field","tenant_field","marker_field"})
  ip=relative_path(ident["path"]); rp=relative_path(resource["path"])
  ...
  if type(repetitions) is not int or not 1<=repetitions<=3: raise ContractError("repetitions_must_be_one_to_three")
  from .action_contract import ActionContract
  action=ActionContract.parse(raw.get("action")); normalized=copy.deepcopy(raw); normalized.update(baseline_prompt=baseline,attacks=list(attacks),repetitions=repetitions)
  return cls(_name(raw["name"]),owner,attacker,ip,_field(ident["subject_field"]),_field(ident["tenant_field"]),rp,_field(resource["id_field"]),_field(resource["owner_field"]),_field(resource["tenant_field"]),_field(resource["marker_field"]),_field(raw["response_path"]),baseline,tuple(attacks),repetitions,action,normalized)
