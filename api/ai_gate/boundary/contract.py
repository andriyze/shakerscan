...
class BoundaryContract:
 name:str;owner:Identity;attacker:Identity;identity_path:str;subject_field:str;tenant_field:str;resource_path:str;resource_id_field:str;resource_owner_field:str;resource_tenant_field:str;marker_field:str;response_path:str;baseline_prompt:str;attacks:tuple[str,...];repetitions:int;action_raw:dict[str,Any]|None;indirect_raw:dict[str,Any]|None;source:dict[str,Any]
 @property
 def digest(self):return canonical_hash(self.source)
 @classmethod
 def parse(cls,raw):
  raw=_keys(raw,{"version","name","owner","attacker","identity","resource","response_path","baseline_prompt","attacks","repetitions","action","indirect"},{"version","name","owner","attacker","identity","resource","response_path"})
  ...
  action=copy.deepcopy(raw.get("action"));indirect=copy.deepcopy(raw.get("indirect"));normalized=copy.deepcopy(raw);normalized.update(baseline_prompt=baseline,attacks=list(attacks),repetitions=reps)
  return cls(_name(raw["name"]),owner,attacker,ip,_field(ident["subject_field"]),_field(ident["tenant_field"]),rp,_field(res["id_field"]),_field(res["owner_field"]),_field(res["tenant_field"]),_field(res["marker_field"]),_field(raw["response_path"]),baseline,tuple(attacks),reps,action,indirect,normalized)
