...
async def execute_multiturn_contract(scenario,contract):
 ...
 for index,prompt in enumerate(contract.turns):
  body=render(scenario.template,{"prompt":prompt,"session_id":session_id,"principal_id":c.attacker.subject,"principal_tenant_id":c.attacker.tenant})
  if any(m in json.dumps(body,sort_keys=True) for m in scenario.markers.values()):raise ContractError("protected_marker_would_be_sent_to_multiturn_chat")
