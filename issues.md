...
| S13 | Every navigation prefetches all fourteen sidebar routes; each prefetch is answered 307 then 200 by the Next server, and over a slow link the page's real API calls queue behind them (the Targets page sat on its skeleton for ~10 s through an SSH tunnel). Same on 2.3.9. | `ui/src/components/Sidebar.tsx` | fixed-unverified (`prefetch={false}` on nav links) |
...
| M7 | Model Intake artifact acquisition ran on a general DAST worker (`worker-16`) although docs say Model Intake jobs are consumed only by the model-intake worker. Routing/doc mismatch. | `api/model_intake/router.py`, docs | open (queue policy routes both Model Intake job kinds to the dedicated worker; needs a live check of which container logged the acquisition on the branch build) |
