"""Execute production run-scope predicates, including opt-in candidates."""
import sqlite3
...
def matched(db, hunt="hunt-a"):
    sql = "SELECT f.id FROM findings f WHERE " + finding_hunt_predicate(1)
    return {r[0] for r in db.execute(sql, {"1": hunt})}
...
def test_opt_in_candidate_union_cannot_escape_hunt_filter():
    db = database()
    sql = "SELECT c.id FROM investigation_candidates c WHERE " + candidate_hunt_predicate(1)
    ids = {r[0] for r in db.execute(sql, {"1": "hunt-a"})}
    assert ids == {"new", "seen"}
