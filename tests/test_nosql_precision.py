...
def test_nosql_json_body_completes_validation_siblings_before_differential(monkeypatch):
    calls = []
    ...
    assert result["validation_fields_added"] == [{
        "url": "https://example.test/api/reviews/search",
        "method": "POST",
        "fields": ["contextId"],
    }]
    assert result["endpoint_attempts"][0]["validation_fields_added"] == ["contextId"]
