...
def test_multi_node_doc_is_build_spec_and_honest_about_fleet_status():
    ...
    assert "implementation complete; 0.8.10 broker physical-acceptance renewal pending" in text
    assert "WireGuard remains preview code" in text
    assert "outside the 0.8.10 supported deployment boundary" in text
