...
def test_device_schema_session_roundtrip_and_graph_owner(monkeypatch, kind):
    ...
    async def scenario():
        ...
        try:
            ...
            await conn.execute(HUNT_SERVICE_SCHEMA_SQL)
            owner, target_id, profile_id, action, graph_id = [uuid4() for _ in range(5)]
            ...
            target = TargetBinding(target_id=str(target_id), target_kind=kind, canonical_host="fixture.test",
                                   allowed_origins=("https://fixture.test:8443",), allowed_addresses=("127.0.0.1",), scope_receipt_id="scope")
            ...
            await conn.execute("INSERT INTO hunt_runs VALUES($1,$2,$3,$4,'active','{}','{}')",
                               owner, None if kind == "device" else target_id, target_id if kind == "device" else None, kind)
