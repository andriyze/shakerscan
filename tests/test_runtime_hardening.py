...
def test_fleet_ui_is_capability_driven_and_keeps_remote_capacity_last():
    ...
    # Capacity wording is asserted behaviourally by the UI suite
    # (ui/tests/fleet-workflow.test.mjs) against workerCapacityLabel, which
    # renders real counts. Duplicating the exact template here made this test
    # fail on rewording, and one of the strings it required had been
    # deliberately removed -- the UI suite asserts its absence.
    assert "workerCapacityLabel(" in dashboard
    ...
    # Capability gate, not the element it happens to render: New Scan may
    # surface Fleet as a card or a link, but only when Fleet is enabled.
    assert "workerStats?.fleet?.enabled &&" in new_scan
