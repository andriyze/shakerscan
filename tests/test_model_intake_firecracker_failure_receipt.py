...
def test_local_pem_can_record_production_runtime_but_cannot_claim_production_pass(tmp_path, monkeypatch):
    key_file = tmp_path / "runner-key.pem"
    key_file.write_text("test-only-key")
    monkeypatch.setattr(firecracker_runner_module, "LocalPemSigner", lambda _key: object())
    monkeypatch.setattr(
    ...
    result = runner.execute_and_sign(request)

    assert result["payload"]["status"] == "INCOMPLETE"
    assert result["payload"]["receipt_signer_trust"] == "non_production_local_pem"
    assert result["payload"]["observations"]["receipt_signer_trust"] == "non_production_local_pem"
