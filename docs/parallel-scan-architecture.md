...
single target campaign timeline from scheduler, schedule, active-scan, and activity facts. Dashboard
Product Status cards now expose API-backed blocker/running/stale counts and quick links across DAST,
ASM, AI Gate, Model Intake, exceptions, deployment gates, and workers. The next operator gap is safe
remediation flow and cross-page campaign-quality agreement, not more parallel-scan knobs. AI Gate
...
work is action depth: safe remediation entry points and family-aware quality agreement across
dashboard, scan detail, `/asm/gaps`, and the target timeline.
