...
digest-pinned physical acceptance and fault matrix. Follow the [operator guide](multi-node-guide.md); the design authority is
...
preview product surface for 0.8.17. Model Intake is release-gated for deterministic static review,
...
- **Admission lifecycle** — signs the complete decision, registers it durably, enforces exact subjects at
  ...
  The core verifier accepts only an exact signed `allow`; HTTP verification also requires an active matching
  registry record. Mutation endpoints require operator authentication, and global operations require explicit
  confirmation plus a change receipt. External deployment systems must integrate the verification boundary.
...
| Scanner modules | 106 | `scanner/scanner_tools/` |
...
| `device-hunt` | Direct a bounded AI investigation of one registered connected device through the device-agent API. Use for requests such as investigate, hunt, or autonomously assess a TV, camera, printer, router, NAS, appliance, or other connected device. Do not use for ordinary Web DAST, read-only explanation without new traffic, or fleet-wide campaigns. | `skills/device-hunt/SKILL.md` |
