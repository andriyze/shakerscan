...
evidence. The backend foundations are ahead of the operator workflow: AI red-team scan-level replay
and saved Model Intake trust-anchor management are the next product priorities. First-pass
next-action / skip-reason state is now live: ASM policy/gaps/improve/activity return `scheduler_state`, and
...
campaign review panel from the stored coverage matrix/evidence manifest, but scan-level rerun/replay
and cross-run campaign history are still follow-ups.
