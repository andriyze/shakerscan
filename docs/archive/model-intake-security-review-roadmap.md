# Archived Model Intake security review and implementation roadmap

> **Historical implementation ledger (archived 2026-08-29).** Completed, configured, proposed, and
> externally pending controls are interleaved below. Use
> [`../model-intake-security-review-roadmap.md`](../model-intake-security-review-roadmap.md) for the
> current supported boundary and acceptance rules.
...
| Engineer-first summary and detailed report | Implemented across UI/JSON/HTML/SARIF with compatibility schema `model-intake-corporate-report/v2` | The first screen identifies the pinned model/revision/digests, separates control counts from scanner counts, and names the exact controller stop reason and next action. The detailed section adds a scanner coverage matrix, bounded repository file manifest, per-tool scanned/skipped file lists, exact dependency denominators, aggregate scan findings that explain blocking status, operation-versus-containment results, bounded Firecracker timelines, and the evidence-backed check catalog. Clean-vulnerability wording is emitted only after the exact resolved package set was actually checked. | Add organization-specific policy/remediation text only when useful; it may not hide or relabel a non-pass |
