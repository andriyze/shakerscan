...
**Status:** Design authority + implementation complete; 0.8.12 broker physical-acceptance renewal pending. The fan-out
...
worker-loss/reclaim acceptance. Because 0.8.12 carries the Fleet lease-polling change, that receipt is historical
and must be renewed on the frozen 0.8.12 SHA before publication. WireGuard physical acceptance is deferred, so WireGuard remains preview code
and is explicitly outside the 0.8.12 supported deployment boundary. Redis Stream
...
durable per-node lifecycle events are implemented. The broker path is the 0.8.12 production
...
| Node identity, enrollment, join tokens, heartbeat, credential rotation/revocation, CA bootstrap, overlay TLS edge, `nodes` table | **Foundation built and broker-tested on two VPSs** — broker fault injection is the 0.8.12 release gate; WireGuard physical acceptance is separately deferred | `fleet.py`; `/fleet/*`; `fleet-edge`; `nodes`, `node_join_tokens`, `node_credentials` |
...
- The node identity/enrollment/overlay and leased/acked/reclaimable queue are now implemented. The
  ...
  0.8.12 must renew it because lease polling changed. WireGuard is deliberately deferred rather than
...
and the idempotent `run_schema_migrations` path in `api/retest_contract.py`, following the 0.8.12
...
|---|---|
| Overlay first or broker first? | **Resolved for 0.8.12:** broker is the supported production transport. WireGuard is built preview code pending separate physical acceptance. |
...
1. **0.8.12 broker gate:** run `shakerscan fleet accept` against two digest-pinned remote workers and
