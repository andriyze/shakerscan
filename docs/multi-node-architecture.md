...
**Status:** Design authority + implementation complete; 2.0.0 broker physical-acceptance renewal pending. The fan-out
...
and is explicitly outside the 2.0.0 supported deployment boundary. Redis Stream
...
- The node identity/enrollment/overlay and leased/acked/reclaimable queue are now implemented. The
  ...
  2.0.0 must renew it as operational evidence because the canonical execution contract changed. WireGuard is deliberately deferred rather than
...
and the idempotent `run_schema_migrations` path in `api/retest_contract.py`, following the 2.0.0
...
|---|---|
| Overlay first or broker first? | **Resolved for 2.0.0:** broker is the supported production transport. WireGuard is built preview code pending separate physical acceptance. |
...
1. **2.0.0 broker evidence:** run `shakerscan fleet accept` against two digest-pinned remote workers and
