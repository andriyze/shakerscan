...
and is explicitly outside the 0.8.2 supported deployment boundary. Redis Stream
...
durable per-node lifecycle events are implemented. The broker path satisfies the 0.8.2 production
...
| Node identity, enrollment, join tokens, heartbeat, credential rotation/revocation, CA bootstrap, overlay TLS edge, `nodes` table | **Foundation built and broker-tested on two VPSs** — broker fault injection is the 0.8.2 release gate; WireGuard physical acceptance is separately deferred | `fleet.py`; `/fleet/*`; `fleet-edge`; `nodes`, `node_join_tokens`, `node_credentials` |
...
and the idempotent `run_schema_migrations` path in `api/retest_contract.py`, following the 0.8.2
...
|---|---|
| Overlay first or broker first? | **Resolved for 0.8.2:** broker is the supported production transport. WireGuard is built preview code pending separate physical acceptance. |
...
1. **0.8.2 broker gate:** run `shakerscan fleet accept` against two digest-pinned remote workers and
