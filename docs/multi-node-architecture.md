...
**Status:** Design authority + implementation complete; broker release-candidate physical acceptance passed. The fan-out
...
A two-node HTTPS-broker deployment has passed non-destructive topology/storage/lease preflight,
digest-pinned cross-node execution with centralized results and artifacts, and destructive physical
worker-loss/reclaim acceptance on an exact release-candidate SHA. The receipt must be rerun if the
frozen release changes Fleet execution behavior. WireGuard physical acceptance is deferred, so WireGuard remains preview code
and is explicitly outside the 0.8.3 supported deployment boundary. Redis Stream
...
durable per-node lifecycle events are implemented. The broker path satisfies the 0.8.3 production
gate; WireGuard remains outside that supported boundary.
...
| Node identity, enrollment, join tokens, heartbeat, credential rotation/revocation, CA bootstrap, overlay TLS edge, `nodes` table | **Foundation built and broker-tested on two VPSs** — broker fault injection is the 0.8.3 release gate; WireGuard physical acceptance is separately deferred | `fleet.py`; `/fleet/*`; `fleet-edge`; `nodes`, `node_join_tokens`, `node_credentials` |
...
- The node identity/enrollment/overlay and leased/acked/reclaimable queue are now implemented. The
  complete digest-pinned broker scan and failure-injection matrix passed on an exact release-candidate
  SHA. WireGuard is deliberately deferred rather than represented as accepted.
...
and the idempotent `run_schema_migrations` path in `api/retest_contract.py`, following the 0.8.3
...
|---|---|
| Overlay first or broker first? | **Resolved for 0.8.3:** broker is the supported production transport. WireGuard is built preview code pending separate physical acceptance. |
...
1. **0.8.3 broker gate:** run `shakerscan fleet accept` against two digest-pinned remote workers and
