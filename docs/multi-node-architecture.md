...
and the idempotent `run_schema_migrations` path in `api/retest_contract.py`, following the 0.8.17
...
|---|---|
| Overlay first or broker first? | **Resolved for 0.8.17:** broker is the supported production transport. WireGuard is built preview code pending separate physical acceptance. |
