...
- **`run_tool`** — argv-templated scanner runs (`httpx` read-only; `nuclei` / `katana` / `ffuf` are
  ...
  the subprocess is wall-clock killed. A successful run receives a `scan_N` evidence ref and a
  scanner-specific typed observation set. Nuclei matches remain candidates, not proof; Katana,
  ffuf, and httpx output becomes bounded route, discovery, and fingerprint observations. Query
  values are removed from the planner-facing typed records. Execution is isolated from the API:
  the API queues a declarative request, and a regular worker independently revalidates the host,
  rebuilds the fixed argv, owns cancellation/timeout, and returns bounded redacted observations.
...
request ceiling of `min(400, iters·12)`, an active-action ceiling of `min(24, iters)`, and ≤ 12 tool
calls per turn. Direct HTTP consumes one request. Before an external scanner starts, the hunt now
reserves that scanner template's fail-closed maximum wire-request allowance (httpx 4, Katana 150,
ffuf 220, Nuclei 450); if the reservation does not fit, the tool does not run. Receipts and final
results expose the reserved amount, exact actual count where a scanner emits a trustworthy counter,
and an observed minimum otherwise. Exact settlement refunds unused reservation; lower-bound or
unavailable settlement remains conservatively charged. This conservative accounting never presents
result rows as total traffic. Scanner argv templates also enforce rate,
duration, concurrency, and subprocess ceilings.
...
human-triaged row. Each compatibility finding is paired with a durable, non-authoritative
investigation candidate keyed by target, family, and canonical operation. Supported web candidates
...
1. **Broader exact scanner counters** — reservation, metering quality, worker isolation, and exact
   refund are implemented; add native exact request counters for scanner versions that currently
   expose only result records, without treating those records as total traffic.
