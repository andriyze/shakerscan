...
API reads/writes and the instance's available MCP/Hunt capabilities run on the server; existing target authorization, budgets, credential admission and protected administrative endpoints still apply. This is not unrestricted remote shell access, and it does not add remote Docker start/stop/update commands. Those lifecycle commands still run on the server. `shakerscan agent`'s saved, authenticated connection flow is unchanged; use the MCP registration above for an unauthenticated LAN OSS instance.
