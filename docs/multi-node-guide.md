...
> **0.8.5 support boundary:** production Fleet support is the outbound-only HTTPS `broker`
> transport. The `wireguard` workflow is an operator preview and has not yet passed its required
> physical two-host acceptance. Do not use WireGuard mode for a 0.8.5
...
|---|---|---|---|
| `broker` | Supported in 0.8.5; use for owned or customer-hosted workers | A node credential and one job-scoped lease at a time; no database, Redis, or object-store credentials | Worker needs outbound HTTPS only |
...
Broker mode is the supported 0.8.5 path and has the smaller worker trust boundary. WireGuard is the
