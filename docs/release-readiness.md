...
channel until 0.8.15 passes complete clean two-VPS post-publication acceptance.
...
ShakerScan 0.8.15 is a trusted-operator, self-hosted security scanner.
...
- Fleet production support is the outbound-only HTTPS `broker` transport. Built-in WireGuard remains
  preview code outside the 0.8.15 support boundary until it passes a separate physical acceptance
  matrix.
...
Run every item against the exact commit intended for `v0.8.15`.
...
1. Confirm `VERSION`, `docs/releases/0.8.15.md`, and the pending `RELEASES.md` row agree.
2. Merge the exact candidate to `main` without adding an untested merge-only change.
3. Wait for required `main` checks, then create annotated tag `v0.8.15` on that exact commit.
...
- [ ] Clean-install `0.8.15` into an empty home and verify doctor, status, UI/API, MCP, agent launch,
