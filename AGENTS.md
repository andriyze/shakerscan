...
- **Credentials (`/credentials`)**: create, rotate, inspect, and deactivate encrypted profiles bound to an exact Web, API, network, or device target. Public responses are metadata-only; profiles support primary, secondary, service, and SSH principal slots plus optional capability and expiry bounds. Legacy Web profiles are backfilled and transactionally mirrored into this store during the V2 compatibility window; execution still decrypts only after worker-side target and approval validation.
