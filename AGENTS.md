...
- **Hunt (`/hunt`, history at `/hunts`)**: launch and review one target-kind-aware investigation through `/hunts`, selecting exact-target generic primary, secondary, service, and SSH credential profiles without exposing their values. History supports target/search/status/kind/budget filters, sorting, and pagination. The current Codex/Claude/OpenCode session owns planning. The 31-entry methodology catalog stays server-side: a Hunt receives at most three compact suggestions and loads one full method only when objective or observed-stack evidence makes it relevant. Runtime bind/unbind is descriptive validation: it never changes scope, authority, capabilities, or budget, and it is not a safety fence around the run. ShakerScan enforces target scope, approval, multidimensional budgets, capability execution, evidence provenance, and deterministic proof promotion. `/deep-hunt` and device-agent pages redirect for compatibility.
...
- **Removed UI surfaces (2.0.0)**: `/interactive`, `/exceptions`, and
  `/settings/ai-ops-router` no longer have pages. Their APIs remain and are reachable by
  agents: the `/session*` interactive routes, `/finding-exceptions*`, and
  `POST /ai/ops/route`. Command Arsenal executes authz replay through an interactive
  session and dispatches `finding_exception.lifecycle_sweep` into the exceptions module,
  so neither API can be removed without removing those Arsenal capabilities with it.
...
target-bound approval receipt. Starting with no methodology is normal. During the run, call
`POST /hunts/{id}/skills/suggestions` with concise observed signals; it returns at most three
metadata-only suggestions. Fetch exactly one at `/hunts/{id}/skills/{skill_id}/read`, then bind it
explicitly if used. Suggestions and binding never grant, remove, or resize authority. Binding validates that the run already has every required capability; it does not constrain the run to that methodology or reduce its existing capability set. The response is a filtered capability manifest plus a compact skills
