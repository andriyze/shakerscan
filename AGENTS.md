...
- **Fleet (`/fleet`)**: inspect joined worker-node health, capacity, heartbeat, egress, image/state drift, recent attributed scans/shards, and lifecycle audit events; distribute a fleet-wide worker target by capacity, scale individual nodes, drain/resume, roll digest-pinned images, and revoke credentials. Remote lifecycle actions accept an operator token kept only for the browser session.
