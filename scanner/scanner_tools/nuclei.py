...
def _should_early_stop(findings: list[dict], signals: dict) -> tuple[bool, str]:
    """
    ...
    """
    critical_count = signals.get("critical_count", 0)
    high_count = signals.get("high_count", 0)

    # Stop if 3+ critical findings
    if critical_count >= 3:
        return True, f"Found {critical_count} critical vulnerabilities"

    # Stop if 5+ high severity findings
    if high_count >= 5:
        return True, f"Found {high_count} high severity vulnerabilities"

    # Stop if many total findings with good severity mix
    if len(findings) >= 10 and (critical_count >= 1 or high_count >= 3):
        return True, f"Found {len(findings)} findings including {critical_count} critical, {high_count} high"
...
async def staged_nuclei_scan(
    ...
    Wave Strategy:
    - Wave 1 (30s): Critical CVEs + tech-specific (100-200 templates)
    - Wave 2 (2min): Misconfig + exposure + signal-based expansion
    - Wave 3 (5min): Injection templates if errors/reflection detected
    - Wave 4 (8min): Full coverage only if promising signals
    ...
    Returns:
        Dict with vulnerabilities, wave stats, and signals
    """
    import sys
    ...
    # =========================================================================
    print(f"[nuclei] Wave 1: Critical + tech-specific templates", file=sys.stderr)
    ...
    print(f"[nuclei] Wave 1 complete: {len(wave1['findings'])} findings in {wave1['duration_seconds']}s", file=sys.stderr)
    ...
    # =========================================================================
    print(f"[nuclei] Wave 2: Expanding based on signals...", file=sys.stderr)
    ...
    if wave2_tags:
        ...
        print(f"[nuclei] Wave 2 complete: {len(wave2['findings'])} findings in {wave2['duration_seconds']}s", file=sys.stderr)
    ...
    if signals.get("sql_errors") or signals.get("xss_reflection") or signals.get("file_inclusion"):
        print(f"[nuclei] Wave 3: Injection-focused templates (signals detected)", file=sys.stderr)
        ...
        if wave3_tags:
            ...
            print(f"[nuclei] Wave 3 complete: {len(wave3['findings'])} findings in {wave3['duration_seconds']}s", file=sys.stderr)
    else:
    ...
    if not early_stopping or _has_promising_signals(signals, all_findings):
        print(f"[nuclei] Wave 4: Deep comprehensive scan", file=sys.stderr)
        ...
        if wave4_tags:
            ...
            print(f"[nuclei] Wave 4 complete: {len(wave4['findings'])} findings in {wave4['duration_seconds']}s", file=sys.stderr)
    else:
